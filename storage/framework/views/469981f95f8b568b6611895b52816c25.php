...
<div class="max-w-5xl mx-auto space-y-6" x-data="slotPicker()" x-init="init(<?php echo e($room->id_room); ?>)">
  ...
  <div class="bg-white rounded-xl shadow p-5 space-y-3">
    <h2 class="font-semibold text-brown-700 flex items-center gap-2 text-lg"><i class="fas fa-calendar-alt text-orange-600"></i> Pilih Tanggal</h2>
    
    <!-- Date Picker Input -->
    <div class="flex flex-col sm:flex-row gap-3">
      <div class="flex-1">
        <label class="block text-sm font-medium text-gray-700 mb-2">Pilih tanggal langsung</label>
        <input type="date" 
               x-model="date" 
               @change="setDate($event.target.value)"
               :min="minDate"
               :max="maxDate"
               class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-xl focus:border-orange-500 focus:outline-none transition text-base">
      </div>
      <div class="flex-1">
        <label class="block text-sm font-medium text-gray-700 mb-2">Info</label>
        <div class="px-4 py-2.5 bg-orange-50 border-2 border-orange-200 rounded-xl">
          <div class="text-sm font-semibold text-gray-800" x-text="dateLabel"></div>
        </div>
      </div>
    </div>
    ...
    <!-- Carousel Tanggal -->
    <div>
      <label class="block text-sm font-medium text-gray-700 mb-2">Atau geser untuk memilih</label>
      <div class="flex items-center gap-2">
        <button type="button" class="px-3 py-2 text-sm rounded-full bg-gray-100 hover:bg-gray-200 shadow-sm flex-shrink-0" @click="page>0 && page--" :disabled="page===0">
          <i class="fas fa-chevron-left"></i>
        </button>
        <div class="flex gap-2 flex-1 justify-between">
          <template x-for="d in allDates.slice(page*7,(page+1)*7)" :key="d.date">
            <button type="button" @click="setDate(d.date)" :class="date===d.date ? 'ring-2 ring-orange-400 bg-orange-50 text-gray-900 shadow-md' : 'bg-white hover:bg-gray-50 text-gray-700'" class="flex-1 px-2 py-2 rounded-2xl border border-gray-200 text-center text-[11px] font-medium transition relative shadow-sm">
        ...
        <button type="button" class="px-3 py-2 text-sm rounded-full bg-gray-100 hover:bg-gray-200 shadow-sm flex-shrink-0" @click="(page+1)*7<allDates.length && page++" :disabled="(page+1)*7>=allDates.length">
...
function slotPicker(){
  return {
    ...
    minDate: '', maxDate: '', dateLabel: '', dateInfo: '', currentMonthLabel: '',
  init(id){
    this.roomId=id;
    this.allDates = this.allDates.map(d=>d);
    // Set min/max date untuk date picker (hari ini sampai 31 Desember 2026)
    const today = new Date();
    this.minDate = today.toISOString().split('T')[0];
    this.maxDate = '2026-12-31';
    // Set date dulu baru fetch manual (hindari race id_room undefined)
    this.date = this.allDates[0]?.date; 
    this.updateDateInfo();
    this.clearSelection();
    ...
    updateDateInfo(){
      ...
      this.dateLabel = days[d.getDay()] + ', ' + d.getDate() + ' ' + months[d.getMonth()] + ' ' + d.getFullYear();
      this.currentMonthLabel = months[d.getMonth()] + ' ' + d.getFullYear();
      const dow = d.getDay();
      this.dateInfo = (dow === 5 || dow === 6 || dow === 0) ? 'Weekend (Harga weekend)' : 'Weekday (Harga normal)';
    },
  setDate(d){ this.date=d; this.updateDateInfo(); this.clearSelection(); this.fetch(); },
    fetch(){
